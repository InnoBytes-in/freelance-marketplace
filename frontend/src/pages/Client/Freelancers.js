...
import dataService from '../../API/dataServive';
...
const Freelancers = () => {
  ...
  useEffect(() => {
    dataService.getFreelancers().then((response) => {
      setIsLoaded(true);
  ...
  function filterCards(freelancers) {
    return freelancers.filter(freelancer => (
      `${freelancer.firstName} ${freelancer.lastName}`.toLowerCase().includes(filter.toLowerCase())));
  }
      ...
      <div>
          ...
          <div className='flex flex-col p-16'>
            ...
            <div className='flex flex-col gap-2'>
              {
                filterCards(freelancers).map((freelancer, index) => (
                  <SocialCard key={index} freelancerData={freelancer} />
                ))
              }
            </div>
          </div>
